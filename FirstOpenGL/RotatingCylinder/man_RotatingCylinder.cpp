...
const int g_num_circle_vertices = 30;
const int g_num_vertices = g_num_circle_vertices * 2;
const int g_num_triangles = g_num_circle_vertices * 2;

glm::vec3 m_positions[g_num_vertices];
glm::vec3 m_normals[g_num_vertices];
glm::vec4 m_colors[g_num_vertices];
glm::ivec3 m_indices[g_num_triangles];
...
int main(int argc, char** argv)
{
	...
	// A.2. positions and normals.
	for (int i = 0; i < g_num_circle_vertices; i++)
	{
		// rotate ( 각도, 축 )
		glm::mat3 R = glm::rotate(glm::radians(360.f / g_num_circle_vertices) * i,
			glm::vec3(0.f, 0.f, 1.f));

		// 밑면
		m_positions[i] = R * m_positions[0];
		m_normals[i] = R * m_normals[0];

		// 윗면
		m_positions[i + g_num_circle_vertices] = m_positions[i] + glm::vec3(0.f, 0.f, 1.f);
		m_normals[i + g_num_circle_vertices] = m_normals[i];
	}
	...
	{
		if (i % 2 == 0)
			m_colors[i] = glm::vec4(1.f, 1.f, 0.f, 1.f);
		else if (i % 2 == 1)
	...
	for (int i = 0; i < g_num_triangles; i++)
	{
		...
		if (i % 2 == 0)
		{
			m_indices[i] = glm::ivec3(i / 2, 1 + (i / 2), m + (i / 2));

			if (i == g_num_triangles - 2)
			{
				m_indices[i][1] = 0;
			}
		}
		...
		{
			m_indices[i] = glm::ivec3(1 + (i / 2), m + 1 + (i / 2), m + (i / 2));

			if (i == g_num_triangles - 1)
			{
				m_indices[i][0] = 0;
				m_indices[i][1] = m;
			}
